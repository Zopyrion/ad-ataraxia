...
async function process_config(path) {
    ...
    for await (const line of rl) {
        if(line[0] === "#"){
            config_key = line.slice(1, line.length).trim(" ").toLowerCase();
            config[config_key] = {};
        } else {
            const pair = line.split("=");
            if(pair[1].length > 2){
                config[config_key][pair[0]] = pair[1];
            }
        }
    }
}
...
async function process(){
    ...
    try {
        ...
        await process_config(CONFIG_PATH);
...
function cleanup(){
    ...
    fs.writeFileSync(CONFIG_PATH, conf);
